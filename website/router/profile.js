...
let iconURL = `https://cdn.discordapp.com/avatars/${req.user.id}/${req.user.avatar}.png?size=128`;
          if (req.user.avatar && req.user.avatar.startsWith("a_")) iconURL = `https://cdn.discordapp.com/avatars/${req.user.id}/${req.user.avatar}.gif?size=128`;

        
  let coins = await client.db.fetch(`coins_${req.user.id}`)
  if(coins == null) coins = "0";
  
  let likes = await client.db.fetch(`likes_${req.user.id}`)
  if(likes == null) likes = "0";
  
  let note = await client.db.fetch(`note_${req.user.id}`)
  if(note == null) note = " ";
  
   var xp = await client.db.fetch(`xp_${req.user.id}`)
        if (xp == null) xp = 1
        var level;
        var nxp;
        var nextLvl;
